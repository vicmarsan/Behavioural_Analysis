...
# File names for every experiment
...
# Read sheet 4 with Y coord time and position and round time (sec)
...
# Speed across X axis per larva per second
...
# Read sheet 2 with speeds and round time (sec)
setwd("C:/Users/viky/Desktop/TFM/decision making/Navigation - Decision Making/thG@Uempty/n_1ul1000EA_600s@n/")
for (file_name in file_names_control_odor) {
...
ggplot(normalized_vel_df, aes(x = population, y = Mean, fill = population)) +
  geom_bar(stat = "identity",position = position_dodge2(width = 0.8, preserve = "single"),color="gray20") +
  geom_errorbar(aes(ymin = Mean - SD, ymax = Mean + SD), width = 0.25, position = position_dodge(width = 0.9),color="gray20") +
  labs(title = "Average speed per population", y = "Average normalized speed (1/s)") +
  scale_fill_manual(values = c("royalblue","royalblue1", "palegreen","palegreen3", "yellow2","yellow1")) + 
  theme_minimal() +
  theme(panel.background = element_rect(fill = "white"),
        panel.grid.major = element_line(color = "grey")) +
...
# Average speeds through time plot

populations_speed_list_time <- list(matrix_list_control_odor_speed, matrix_list_control_free_speed,
                               matrix_list_asyn_odor_speed, matrix_list_asyn_free_speed,
                               matrix_list_LRRK2_odor_speed, matrix_list_LRRK2_free_speed)
speeds_time <- list()

for (population in populations_speed_list_time) {
  results_population_time <- list()
  for (file in population) {
    submatrix_df <- as.data.frame(file)
    submatrix_df <- submatrix_df %>%
      arrange(`time (s)`)
    mean_speeds <- submatrix_df %>%
      group_by(`time (s)`) %>%
      summarize(mean_speed = mean(`speed (mm/s)`))
    results_population_time[[length(results_population_time) + 1]] <- mean_speeds
  }
  speeds_time[[length(speeds_time) + 1]] <- results_population_time
}

# Normalize speeds by average larvae length

normalized_vel_time <- list()

for (i in seq_along(speeds_time)) {
  sublista <- speeds_time[[i]]
  vector <- larvae_avg_length[[i]]
  normalized_vel_sublista <- list()
  for (j in seq_along(sublista)) {
    data_frame <- sublista[[j]]
    valor <- vector[[j]]
    data_frame$vel_norm <- data_frame$mean_speed / valor
    normalized_vel_sublista[[j]] <- data_frame
  }
  normalized_vel_time[[i]] <- normalized_vel_sublista
}

# mean speeds and deviation by population

normalized_vel_time_pop <- list()

for (i in 1:6) {
  poblacion_i <- normalized_vel_time[[i]]
  velocidades <- sapply(poblacion_i, function(x) x$vel_norm)
  medias <- rowMeans(velocidades)
  desviaciones <- apply(velocidades, 1, sd)
  df <- data.frame(Time = poblacion_i[[1]]$`time (s)`, MeanVel = medias, SD = desviaciones)
  normalized_vel_time_pop[[i]] <- df
}

names(normalized_vel_time_pop) <- c("Control odour","Control free","aSyn-A53T odour","aSyn-A53T free","LRRK2-G2019S odour","LRRK2-G2019S free")

normalized_vel_time_pop <- lapply(seq_along(normalized_vel_time_pop), function(i) {
  df <- normalized_vel_time_pop[[i]]
  df$population <- names(normalized_vel_time_pop)[i]
  return(df)
})

# All 6 populations

combined_df_time <- do.call(rbind, normalized_vel_time_pop)
combined_df_time <- subset(combined_df_time, Time != 600)

ggplot(combined_df_time, aes(x = Time, y = MeanVel, color = population)) +
  geom_line() +
  geom_ribbon(aes(ymin = MeanVel - SD, ymax = MeanVel + SD), alpha = 0.2) +
  labs(x = "Time (s)", y = "Average normalized speed (1/s)") +
  scale_color_discrete(name = "Population") +
  theme_minimal()

# only odour populations

combined_df_time_odour <- subset(combined_df_time, population != "Control free")
combined_df_time_odour <- subset(combined_df_time_odour, population != "aSyn-A53T free")
combined_df_time_odour <- subset(combined_df_time_odour, population != "LRRK2-G2019S free")

ggplot(combined_df_time_odour, aes(x = Time, y = MeanVel, fill = population, color = population)) +
  geom_ribbon(aes(ymin = MeanVel - SD, ymax = MeanVel + SD), alpha = 0.2) +
  geom_line(linewidth=1) +
  labs(x = "Time (s)", y = "Average normalized speed (1/s)",title = "Average speeds over time") +
  scale_fill_manual(values = c("royalblue1", "palegreen3", "yellow"), name = "Población") +
  scale_color_manual(values = c("royalblue1", "palegreen3", "yellow"), name = "Población") +
  theme_minimal() +
  theme(panel.background = element_rect(fill = "white"),
        panel.grid.major = element_line(color = "grey")) +
  guides(fill = guide_legend(title = NULL), color = guide_legend(title = NULL))

combined_df_time_odour %>%
    group_by(population) %>%
    mean(MeanVel,na.rm=T)

# ANGULAR VELOCITY

population_directories <- c("C:/Users/viky/Desktop/TFM/decision making/Navigation - Decision Making/thG@Uempty/n_1ul1000EA_600s@n/",
                            "C:/Users/viky/Desktop/TFM/decision making/Navigation - Decision Making/thG@Uempty/n_freeNavigation_600s@n",
                            "C:/Users/viky/Desktop/TFM/decision making/Navigation - Decision Making/thG@UaSynA53T/n_1ul1000EA_600s@n",
                            "C:/Users/viky/Desktop/TFM/decision making/Navigation - Decision Making/thG@UaSynA53T/n_freeNavigation_600s@n",
                            "C:/Users/viky/Desktop/TFM/decision making/Navigation - Decision Making/thG@UG2019S/n_1ul1000EA_600s@n",
                            "C:/Users/viky/Desktop/TFM/decision making/Navigation - Decision Making/thG@UG2019S/n_freeNavigation_600s@n")

population_names <- c("control_odor","control_free","aSyn-A53T_odor","aSyn-A53T_free",
                      "LRRK2-G2019S_odor","LRRK2-G2019S_free")
...
combined_results_angular_velocity <- combined_results_angular_velocity %>%
  group_by(Population) %>%
  mutate(angular_velocity = abs(`mean_angle` - lag(`mean_angle`, default = first(`mean_angle`))) / 
           (`time (s)` - lag(`time (s)`, default = first(`time (s)`))))

avg_ang_velocity <- combined_results_angular_velocity %>%
  group_by(Population) %>%
  summarise(mean_ang_velocity = mean(`angular_velocity`, na.rm = TRUE),sd=sd(angular_velocity,na.rm=T))

kruskal.test(angular_velocity ~ Population, combined_results_angular_velocity)
mc <- dunn.test::dunn.test(x = combined_results_angular_velocity$angular_velocity, g = combined_results_angular_velocity$Population, method = "bonferroni")
